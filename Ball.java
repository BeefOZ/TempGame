...
public class Ball implements GameConstants {
    ...
    public void moveBall(BallComponent ballComponent) {
        ...
        for (Ball ballVer : ballComponent.listBall) {
            ...
            // Если не является ли он сам собой (для чего и понадобился id)

            if (inAction == 0)
                if ((Math.sqrt(Math.pow(x - ballVer.x, 2) + Math.pow(y - ballVer.y, 2))) <= radius + ballVer.radius
                        && id != ballVer.id && (ballVer.inAction == 1 || ballVer.inAction == 2)) {
                    ballComponent.score++;
            ...
            if (inAction == 1) {
                ...
                if (radius >= MAXRADIUS) {
                    inAction = 2;
                    dRadius = 0;
                    // запускается таймер, который по прошествии времени жизни, начнёт уменьшать
                    // радиус шарика
    ...
    // таймер, запускаемый по истечении LIFETIME, если радиус шарика достиг
    // максимального:
